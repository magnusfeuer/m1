...
then
    echo "Usage $0 disk"
    echo "Disk is the USB device to setup as a reset stick. (/dev/sda for example)."
    echo "An M1 booted with this stick will be reset to factory settings.."
    exit 255
fi
...
LOG=create_reset.log

echo "$0 "'date' > $LOG
echo "--- Log file can be found in create_reset.log"
...
cp  reset_linuxrc /mnt/loopback/linuxrc
